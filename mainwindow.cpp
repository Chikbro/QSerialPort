    ...
    ui(new Ui::MainWindow)
{
    ...
    ui->Comand->addItem(QLatin1String("0x00"),SEDP::Command::Command_GetPassport);
    ui->Comand->addItem(QLatin1String("0x01"),SEDP::Command::Command_SetPassport);
    ui->Comand->addItem(QLatin1String("0x02"),SEDP::Command::Command_GetInfo);
  ...
  //  Protocol buf {_port};
    ...
    connect(PortNew, SIGNAL(outPort(QByteArray)), this, SLOT(Print(QString)));
    connect(this,SIGNAL(writeData(QByteArray)),PortNew,SLOT(WriteOut(QByteArray)));
    connect(this,SIGNAL(readData(QByteArray)),PortNew,SLOT(ReadInPort()));
    //connect(this,SIGNAL(writeData(QByteArray)),PortNew,SLOT(Protocol::write(QByteArray)));
    thread_New->start();
    //--------------------------------------------------------------------------------
...
//    if (Port::buf.isEmpty())
//    {
//        ui->consol->insertPlainText("Message empty \n");
//    }
//    else
//    {
    writeData(data);
...
QByteArray MainWindow::Com(QByteArray Comand)
{
    Comand=ui->Comand->currentText().toLocal8Bit();
    return Comand;
}
